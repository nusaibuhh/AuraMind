...
--
-- Table structure for table `behavioral_activities`
--

DROP TABLE IF EXISTS `behavioral_activities`;
CREATE TABLE `behavioral_activities` (
  `id` varchar(64) NOT NULL,
  `title` text NOT NULL,
  ...
  PRIMARY KEY (`id`),
  KEY `behavioral_activities_active_difficulty_duration_idx` (`is_active`,`difficulty`,`duration_minutes`),
  CONSTRAINT `behavioral_activities_difficulty_check` CHECK (`difficulty` in ('tiny','easy','moderate')),
  CONSTRAINT `behavioral_activities_duration_check` CHECK (`duration_minutes` > 0)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

INSERT INTO `behavioral_activities` VALUES
('act_water_plant','Water a plant','Give a plant some water or notice a plant nearby.','Self-care','tiny',2,1,'2026-08-28T00:00:00'),
('act_walk_5min','Walk for 5 minutes','Step outside or walk around your room for just 5 minutes.','Physical','tiny',5,1,'2026-08-28T00:00:00'),
('act_drink_water','Drink a glass of water','Slowly sip a fresh glass of water to refresh yourself.','Self-care','tiny',2,1,'2026-08-28T00:00:00'),
('act_favorite_song','Listen to one favorite song','Put on one song you love and simply listen.','Enjoyment','tiny',4,1,'2026-08-28T00:00:00'),
('act_message_friend','Send a message to a friend','Say hello or send a simple note or emoji to someone.','Social','tiny',2,1,'2026-08-28T00:00:00'),
('act_sit_outside','Sit outside for 5 minutes','Feel the fresh air, warmth, or breeze for 5 minutes.','Outdoor','tiny',5,1,'2026-08-28T00:00:00'),
('act_tidy_area','Tidy one small area','Organize one spot like your desk or a single drawer.','Productivity','tiny',3,1,'2026-08-28T00:00:00'),
('act_take_shower','Take a warm shower','Enjoy a warm and refreshing shower with no rush.','Self-care','easy',10,1,'2026-08-28T00:00:00'),
('act_gratitude','Write down one thing you are grateful for','Note down one small thing that brought you comfort or peace.','Relaxation','tiny',3,1,'2026-08-28T00:00:00'),
('act_stretch','Gentle 3-minute stretch','Do a few easy shoulder rolls and gentle body stretches.','Physical','tiny',3,1,'2026-08-28T00:00:00'),
('act_deep_breaths','Take 5 slow, deep breaths','Inhale deeply, hold gently, and exhale fully.','Relaxation','tiny',2,1,'2026-08-28T00:00:00'),
('act_look_sky','Look at the sky or trees','Spend a moment watching the clouds, trees, or sky.','Outdoor','tiny',3,1,'2026-08-28T00:00:00'),
('act_read_page','Read 2 pages of a book','Read just a couple of pages of any book or article you like.','Enjoyment','tiny',5,1,'2026-08-28T00:00:00'),
('act_mindful_tea','Mindful tea or warm drink','Make a warm drink and focus on the aroma and warmth.','Relaxation','easy',7,1,'2026-08-28T00:00:00'),
('act_kind_thought','Think of one kind thought for yourself','Remind yourself that doing your best each day is enough.','Self-care','tiny',2,1,'2026-08-28T00:00:00');

--
-- Table structure for table `behavioral_daily_tasks`
--

DROP TABLE IF EXISTS `behavioral_daily_tasks`;
CREATE TABLE `behavioral_daily_tasks` (
  `id` varchar(64) NOT NULL,
  ...
  CONSTRAINT `behavioral_daily_tasks_status_check` CHECK (`status` in ('pending','completed','skipped')),
  CONSTRAINT `behavioral_daily_tasks_mood_before_check` CHECK (`mood_before` is null or `mood_before` between 1 and 5),
  CONSTRAINT `behavioral_daily_tasks_mood_after_check` CHECK (`mood_after` is null or `mood_after` between 1 and 5)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

--
-- Table structure for table `savoring_logs`
--

DROP TABLE IF EXISTS `savoring_logs`;
CREATE TABLE `savoring_logs` (
  ...
  CONSTRAINT `savoring_logs_status_check` CHECK (`status` in ('draft','completed'))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

--
-- Table structure for table `savoring_entries`
--

DROP TABLE IF EXISTS `savoring_entries`;
CREATE TABLE `savoring_entries` (
  ...
  CONSTRAINT `savoring_entries_order_check` CHECK (`entry_order` between 1 and 3)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
