...
class _AdminPanelScreenState extends State<AdminPanelScreen> {
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() { _loggedIn = true; _data = data; });
    } catch (e) {
  ...
  Future<void> _deleteReportedComment(Map<String, dynamic> report) async {
    final commenter = Map<String, dynamic>.from(report['commenter'] as Map? ?? {});
    final name = commenter['name']?.toString() ?? 'this user';
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Delete reported comment?'),
        content: Text('Delete the reported comment authored by $name? This action removes the comment from the community and cannot be undone.'),
        actions: [
  ...
  Widget build(BuildContext context) {
    ...
    final moderation = List<Map<String, dynamic>>.from(
      (data['moderation_queue'] as List? ?? []).map((e) => Map<String, dynamic>.from(e)),
    );
    final practitioners = List<Map<String, dynamic>>.from(
      (data['practitioner_queue'] as List? ?? []).map((e) => Map<String, dynamic>.from(e)),
    );
    ...
    final commentReports = List<Map<String, dynamic>>.from(
      (data['comment_reports'] as List? ?? []).map((e) => Map<String, dynamic>.from(e)),
    );
    final policies = List<Map<String, dynamic>>.from(
      (data['moderation_policies'] as List? ?? []).map((e) => Map<String, dynamic>.from(e)),
    );
    final settings = List<Map<String, dynamic>>.from(
      (data['system_settings'] as List? ?? []).map((e) => Map<String, dynamic>.from(e)),
    );
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            onPressed: () async {
              await _api.logout();
              if (mounted) setState(() { _loggedIn = false; _data = null; });
            },
        ...
        child: ListView(
          ...
          children: [
            ...
            if (moderation.isEmpty)
              const Card(child: ListTile(title: Text('No pending moderation items.')))
            else
              ...moderation.map((item) => Card(
                child: Padding(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('${item['category']} · ${(item['confidence'] as num).toStringAsFixed(2)}',
                      ...
                      Wrap(
                        spacing: 8,
                        children: [
                          OutlinedButton(onPressed: () => _moderation(item['id'], 'approved'), child: const Text('Approve')),
                          FilledButton(onPressed: () => _moderation(item['id'], 'hidden'), child: const Text('Hide')),
                          OutlinedButton(onPressed: () => _moderation(item['id'], 'rejected'), child: const Text('Reject')),
                        ],
                      ),
                    ],
                  ),
            ...
            if (reports.isEmpty)
              const Card(child: ListTile(title: Text('No community reports recorded.')))
            else
              ...reports.take(20).map((report) => Card(
                child: ListTile(
                  leading: const Icon(Icons.flag_outlined),
                  title: Text(report['reason'] as String),
                  subtitle: Text('Anonymous report · post ${report['post_id']}'),
                ),
              )),
            const SizedBox(height: 20),
            _sectionTitle('Reported comments — safety review'),
            if (commentReports.isEmpty)
              const Card(child: ListTile(title: Text('No reported comments recorded.')))
            else
                    ...
                    child: Column(
                      ...
                      children: [
                        Text('Report: ${report['reason'] ?? 'Comment report'}',
                            style: const TextStyle(fontWeight: FontWeight.w800)),
                        const SizedBox(height: 8),
                        Text(report['comment']?.toString() ?? 'Comment content unavailable'),
                        const SizedBox(height: 12),
                        Text('Comment author: ${commenter['name'] ?? 'Unknown user'}',
                            style: const TextStyle(fontWeight: FontWeight.w700)),
                        Text('Email: ${commenter['email'] ?? 'No email available'}'),
                        Text('User ID: ${commenter['user_id'] ?? 'Unavailable'}',
            ...
            if (practitioners.isEmpty)
              const Card(child: ListTile(title: Text('No pending registrations.')))
            else
              ...practitioners.map((item) => Card(
                child: Padding(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(item['name'] as String, style: const TextStyle(fontWeight: FontWeight.w800)),
                      ...
                      Wrap(
                        spacing: 8,
                        children: [
                          FilledButton(onPressed: () => _practitioner(item['id'], 'approved'), child: const Text('Approve & publish')),
                          OutlinedButton(onPressed: () => _practitioner(item['id'], 'rejected'), child: const Text('Reject')),
                        ],
                      ),
                    ],
                  ),
            ...
            ...policies.map((p) => Card(
              child: ListTile(
                ...
                subtitle: Text('Threshold: ${(p['threshold'] as num).toStringAsFixed(2)}'),
                trailing: Switch(
            ...
            ...settings.map((s) => Card(
              child: ListTile(
                ...
                onTap: () => _updateSetting(s['key'] as String, s['value'] as String),
              ),
            )),
          ],
        ),
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 8),
                    const Text('Administrator credentials are checked by the FastAPI backend.'),
                    const SizedBox(height: 20),
                    TextField(
                      controller: _email,
                    ...
                    SizedBox(
                      ...
                      child: FilledButton.icon(
                        ...
                        label: Text(_loading ? 'Signing in...' : 'Sign in as administrator'),
                      ),
                    ),
                    const SizedBox(height: 12),
                    const Text(
                      'Local demo default: admin@auramind.local / Admin@1234. '
                      'Set ADMIN_EMAIL and ADMIN_PASSWORD in FastAPI/.env before deployment.',
